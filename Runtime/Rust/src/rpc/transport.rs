...
pub trait TransportProtocol {
    ...
    fn send(&self, datagram: &Datagram) -> Pin<Box<dyn Future<Output = TransportResult>>>;
}

impl<D: Deref<Target = T>, T: TransportProtocol> TransportProtocol for D {
    fn set_handler(&self, recv: TransportHandler) {
        self.deref().set_handler(recv)
    }
