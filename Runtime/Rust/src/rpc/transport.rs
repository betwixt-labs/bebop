...
pub trait TransportProtocol {
    // TODO: the solution to lifetimes here is we need Datagram to be a concrete type so the
    //  functions can track the lifetime rather than the trait.
    //  This does mean we will have to generate the bebop for Datagram in the runtime.

    fn set_handler<
        F: 'static + Fn(Self::Datagram) -> FReturn,
        FReturn: 'static + Future<Output = ()>,
    >(
        &mut self,
        recv: F,
    ) {
    ...
    fn set_handler_boxed(&mut self, recv: TransportHandler<Self::Datagram>);

    async fn send(&self, datagram: &Self::Datagram) -> TransportResult;
}
