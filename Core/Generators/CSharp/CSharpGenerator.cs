    ...
    public class CSharpGenerator : Generator
    {
        ...
        private void CompileUnionFamily(IndentedStringBuilder builder, UnionDefinition ud)
        {
            ...
            void CompileIsAs()
            {
                ...
                foreach (var branch in ud.Branches)
                {
                    builder.AppendLine($"public bool Is{branch.Definition.ClassName()} => _discriminator is {branch.Discriminator};");
                    builder.AppendLine(
                        $"public T{branch.GenericIndex()} As{branch.Definition.ClassName()} => _discriminator is {branch.Discriminator} ? _value{branch.GenericIndex()} : throw new global::System.NotImplementedException($\"Cannot return as {branch.Definition.ClassName()} as result is T{branch.GenericIndex()}\");").AppendLine();
                }
                builder.AppendLine("#endregion");
            }
