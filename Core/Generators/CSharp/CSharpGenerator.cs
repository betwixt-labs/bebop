...
namespace Core.Generators.CSharp
{
    public class CSharpGenerator : Generator
    {
        ...
        private static readonly string GeneratedAttribute = $"[global::System.CodeDom.Compiler.GeneratedCode(\"{ReservedWords.CompilerName}\", \"{ReservedWords.CompilerVersion}\")]";
...
//------------------------------------------------------------------------------";
        ...
        private string CompileSizeAssignment(TypeBase type, string target, bool isAccurate, int depth = 0, int indentDepth = 0)
        {
            ...
            return type switch
            {
                ...
                DefinedType => $"{outerVariable} {target}.{(isAccurate? "GetByteCount()" : "GetMaxByteCount()")};",
        ...
        private string CompileGetByteCount(TopLevelDefinition definition, bool isAccurate)
        {
            ...
            builder.AppendLine($"public int {(isAccurate ? "GetByteCount" : "GetMaxByteCount")}() {{").Indent(indentStep);
            ...
            else if (definition is UnionDefinition union)
            {
                builder.AppendLine("byteCount += sizeof(uint) + sizeof(byte);");
            }
            ...
            foreach (var definition in Schema.Definitions.Values)
            {
                ...
                else if (definition is TopLevelDefinition)
                {
                    if (definition is FieldsDefinition fd)
                    {
                        ...
                        builder.AppendLine($"public const uint MinimalEncodeSize = {fd.MinimalEncodedSize(Schema)};");
                        if (fd is MessageDefinition)
                        {
                        ...
                        if (fd.Fields.Count > 0)
                        {
                            ...
                            builder.AppendLine($"protected {definition.BaseClassName()}({nulllabilityAttribute} {definition.BaseClassName()}{(fd is MessageDefinition ? "?" : "")} original) => {propertyTuple} = {copyTuple};");
                        ...
                        builder.AppendLine(GeneratedAttribute);
                        builder.AppendLine($"{recordAttribute}");
                        builder.AppendLine($"public sealed partial class {definition.ClassName()} : {PrefixNamespace(definition.BaseClassName())} {{").AppendLine();
                        builder.Indent(indentStep);
                        ...
                        builder.AppendLine("/// <inheritdoc />");
                        builder.AppendLine($"public {definition.ClassName()}() : base() {{ }}").AppendLine();
                        if (fd.Fields.Count > 0)
                        {
                            builder.AppendLine("/// <inheritdoc />");
                            builder.AppendLine($"public {definition.ClassName()}({constructorArguments}) : base({constructorArgumentNames}) {{ }}").AppendLine();
                            builder.AppendLine("/// <inheritdoc />");
                            builder.AppendLine($"public {definition.ClassName()}({nulllabilityAttribute} {definition.ClassName()}{(fd is MessageDefinition ? "?" : "")} original) : base(original) {{ }}").AppendLine();
                        }
                        ...
                        builder.AppendLine();
                        builder.AppendLine(CompileEncodeHelper(definition, ImmutableByteArrayType, "EncodeAsImmutable"));
                        builder.AppendLine();
        ...
        private void CompileUnionFamily(IndentedStringBuilder builder, UnionDefinition ud)
        {
            ...
            void CompileUnionBaseClasses()
            {
                ...
                builder.AppendLine($"public const uint MinimalEncodeSize = {ud.MinimalEncodedSize(Schema)};");
                foreach (var branch in ud.Branches)
                {
                    builder.AppendLine($"internal readonly T{branch.GenericIndex()} _value{branch.GenericIndex()};");
                }
            ...
            void CompileUnionConcreteClass()
            {
                ...
                builder.AppendLine();
                builder.AppendLine(CompileEncodeHelper(ud, ImmutableByteArrayType, "EncodeAsImmutable")).AppendLine();
        ...
        private string TypeName(in TypeBase type, string arraySizeVar = "")
        {
            return type switch
            {
                ...
                DefinedType dt => PrefixNamespace($"{(dt.IsEnum(Schema) ? string.Empty : "Base")}{dt.Name.ToPascalCase()}"),
