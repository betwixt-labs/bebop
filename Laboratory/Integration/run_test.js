...
if (
  shell.exec(
    "dotnet run --project ../../Compiler --include schema.bop build --generator 'cs:schema.cs' --generator 'ts:schema.ts' --generator 'cpp:schema.hpp' --generator 'rust:Rust/src/schema.rs' --generator 'py:Python/src/schema.py'"
  ).code !== 0
) {
...
const languages = [
  {
    name: "Rust",
    cmd: "cargo run --manifest-path Rust/Cargo.toml -q --example decode --",
    file: "rs.enc",
  },
...
for (const decode of languages) {
  for (const encode of languages) {
    ...
    if (shell.exec(`${decode.cmd} ${encode.file}`).code !== 0) {
      shell.echo(
        `\x1b[31mError: ${decode.name} failed to decode ${encode.name}\x1b[0m`
      );
      failed = true;
    }
  }
