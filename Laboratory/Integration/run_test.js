...
shell.exec("dotnet run encode").to("cs.enc");
shell.exec("npx ts-node encode.ts").to("ts.enc");
if (shell.exec(`${cxx} --std=c++17 encode.cpp`).code !== 0) {
  shell.echo(`Error: ${cxx} encode.cpp failed`);
  shell.exit(1);
}
shell.exec("./a.out").to("cpp.enc");
shell.rm("-f", "./a.out");
shell.exec("cargo run --manifest-path Rust/Cargo.toml --example encode").to("rs.enc");
