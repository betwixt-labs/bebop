...
pub mod owned {
    ...
    pub use super::Instrument;#[derive(Clone, Debug, PartialEq)]
    ...
    impl<'raw> ::core::convert::From<super::Song<'raw>> for Song {
        fn from(value: super::Song) -> Self {
            Self {
                ...
                year: value.year,
                performers: value.performers.map(|value| value.iter().map(|value| value.into()).collect()),
            }
        }
    }

    impl<'raw> ::bebop::SubRecord<'raw> for {
        const MIN_SERIALIZED_SIZE: usize = ::bebop::LEN_SIZE + 1;
    ...
    impl<'raw> ::core::convert::From<super::Album<'raw>> for Album {
        fn from(value: super::Album) -> Self {
            match self {
                ...
                super::Album::StudioAlbum {
                    tracks: _tracks,
                }
                => {
                    Self::StudioAlbum {
                        tracks: value.tracks.iter().map(|value| value.into()).collect(),
                    }
                }
                ...
                => {
                    Self::LiveAlbum {
                        tracks: value.tracks.map(|value| value.iter().map(|value| value.into()).collect()),
                        venue_name: value.venue_name.map(|value| value.into()),
                        concert_date: value.concert_date,
                    }
                }
            }
        }
    ...
    #[derive(Clone, Debug, PartialEq)]
    pub struct Library {
        pub albums: ::std::collections::HashMap<String, Album<'raw>>,
    }

    impl<'raw> ::core::convert::From<super::Library<'raw>> for Library {
        fn from(value: super::Library) -> Self {
            Self {
                albums: value.albums.into_iter(|(key, value)| (key.into(), value.into())).collect(),
            }
        }
    }
    ...
    impl<'raw> ::bebop::SubRecord<'raw> for Library {
        const MIN_SERIALIZED_SIZE: usize =
            <::std::collections::HashMap<String, Album<'raw>>>::MIN_SERIALIZED_SIZE;
